...
// that end to end, which needs the vorbis fork's allocation-free DecodeInto:
// it lives on the cog branch only, which builds over that fork.

func openReaderClip(t *testing.T) *oggvorbis.Reader {
	t.Helper()
...
func TestReaderReadAllocatesNothing(t *testing.T) {
	r := openReaderClip(t)
...
func TestReaderSetPositionAllocatesNothing(t *testing.T) {
	r := openReaderClip(t)
	length := r.Length()
